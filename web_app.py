...
# 1. KULLANICI METİN GİRİŞ ALANLARI (EKSTRA STAND NO ALANI EKLENDİ)
st.subheader("📝 Fuar Bilgilerini Girin")
col1, col2 = st.columns(2)
with col1:
    stand_no = st.text_input("Stand No (Dosya Adı Ön Eki - Örn: 2186-01)", "2186-01")
    firma_adi = st.text_input("Firma Adı (Örn: Asis)", "Asis")
    fuar_adi = st.text_input("Fuar Adı (Örn: Innotrans 2026)", "Innotrans 2026")
with col2:
    hall_stand = st.text_input("Hall/Stand (Slayt İçi Konum - Örn: 655)", "655")
    olcu = st.text_input("Ölçü (Örn: 6.0 x 5.0m)", "6.0 x 5.0m(30qm)")
...
def metni_temizle_ve_degistir(paragraph):
    if "{" in paragraph.text and "}" in paragraph.text:
        ...
        text = text.replace("{STAND}", hall_stand)  # Slayt içindeki yere Hall/Stand metni gelir
        text = text.replace("{STAND_NO}", stand_no) # İhtiyaç olursa slayt içinde de {STAND_NO} kullanılabilir
...
def pptx_to_pdf(pptx_path, output_dir):
    try:
        ...
        st.error("PDF'e dönüştürülürken hata oluştu. packages.txt dosyasının varlığından emin olun.")
        return False
...
if st.button("Sunumu Oluştur", type="primary"):
    if sablon_dosya is not None and len(yuklenen_resimler) > 0:
        with st.spinner("Slaytlar ve PDF hazırlanıyor, bu işlem birkaç saniye sürebilir..."):
            ...
            # Tüm katmanlardaki etiketleri değiştir
            for master in prs.slide_masters:
                sekilleri_tara_ve_degistir(master.shapes)
            ...
            st.success("Tebrikler! Sunum ve PDF başarıyla oluşturuldu.")
            
            # --- DOSYA ADLANDIRMA ALGORİTMASI ---
            # Dosya adını bozabilecek geçersiz karakterleri temizliyoruz
            güvenli_stand_no = re.sub(r'[\\/*?:"<>|]', "-", stand_no).strip()
            güvenli_fuar = re.sub(r'[\\/*?:"<>|]', "-", fuar_adi).strip()
            
            # Format: "2186-01 Innotrans 2026 Tasarım"
            olusturulan_dosya_adi = f"{güvenli_stand_no} {güvenli_fuar} Tasarım"
            ...
            with buton_kolonu1:
                st.download_button(
                    ...
                    data=pptx_bytes,
                    file_name=f"{olusturulan_dosya_adi}.pptx",
                ...
                if os.path.exists(pdf_path):
                    ...
                    with buton_kolonu2:
                        st.download_button(
                            ...
                            data=pdf_bytes,
                            file_name=f"{olusturulan_dosya_adi}.pdf",
